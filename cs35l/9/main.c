...
{
	struct arguments *func_args=(struct arguments *)args;

	int nthreads=(*func_args).nthreads;
        int width=(*func_args).width;
        double pixel_dx=(*func_args).pixel_dx;
        int height=(*func_args).height;
        double pixel_dy=(*func_args).pixel_dy;
        int halfSamples=(*func_args).halfSamples;
        double subsample_dx=(*func_args).subsample_dx;
        double subsample_dy=(*func_args).subsample_dy;
        Vec3 *bg_color=(*func_args).bg_color;
        scene_t *scene=(*func_args).scene;
        Vec3 *camera_pos=(*func_args).camera_pos;
        float* image=(*func_args).image;
        int thread_num=(*func_args).thread_num;

	int width_per_thread=width/nthreads;
	int width_limit=width_per_thread*(thread_num+1);

    /* for every pixel */
    for(int px=width_per_thread*thread_num; (px<width_limit) && (px<width); ++px )
    {
        const double x = pixel_dx * ((double)( px-(width/2) ));
        for( int py=0; py<height; ++py )
        {
	    int pos=(px*height+py)*3;
...
*/
    ...
    struct arguments args[128];
    float *image = (float *)malloc(width*height*3*sizeof(float));

    for(int i=0; i<nthreads; i++)
    {
	args[i].nthreads = nthreads;
        args[i].pixel_dx = pixel_dx;
        args[i].pixel_dy = pixel_dy;
        args[i].subsample_dx = subsample_dx;
        args[i].subsample_dy = subsample_dy;
        args[i].width = width;
        args[i].height = height;
        args[i].halfSamples = halfSamples;
        args[i].camera_pos = &camera_pos;
        args[i].scene = &scene;
        args[i].bg_color = &bg_color;
        args[i].image = image;
        args[i].thread_num = i;
    }
